...
class Class {
    ...
    addTargetTag(name) { this._targetTags.add(name); }
    hasTargetTag(name) { return this._targetTags.has(name); }
}
...
saveBridged()
{
    ...
    for (let modelObject of this._objects.values()) {
...
add(object)
{
    ...
    let existing = this._objects.get(name);
